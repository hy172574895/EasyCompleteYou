...
class Operate(scope_.Source_interface):
    def __init__(self):
        """ notes:
        """
        self._name = 'intelephense'
        self.lsp_name = 'intelephense'
        self.languageId = 'php'
    ...
    def _check(self, version):
        ...
        if self._get_lsp_setting(version['lsp_setting']):
            g_logger.debug("Missing lsp_setting")
            return False
        self._start_server()
    ...
    def _read_regex(self):
        try:
            if 'refresh_pattern' in self.lsp_setting['config'] and \
                    self.lsp_setting['config']['refresh_pattern'] != '':
                self.regex = repr(self.lsp_setting['config']['refresh_pattern'])
        except:
            pass
    ...
    def _did_open_or_change(self, uri, text, document_id):
        ...
        if uri not in self._did_open_list:
            return_id = self._lsp.didopen(uri, self.languageId, text, version=0)
